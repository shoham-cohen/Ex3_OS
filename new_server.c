...
int main(void)
{
    ...
    while(1) {  // main accept() loop
        ...
        */
        int succeed = pthread_create(&new_thread, NULL, &start_routine, new_fd);
        if(succeed != 0){
            printf("error");
            return -1;
        }
...
void* start_routine(int new_fd){
    //print the client identification
    printf("we are handeling with client number %d\n",new_fd);
    //copy from the fork section
    if (send(new_fd, "Hello, world!", 13, 0) == -1)
        perror("send");
    close(new_fd);
    // print that we are done with this client and return NULL because this method has
    //to return void*
    printf("we are done with client number %d\n",new_fd);
